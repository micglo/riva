...
{
    public class UserRoomForRentAnnouncementPreferenceChangedDomainEventHandlerTest
    {
        private readonly Mock<IDomainEventStore> _domainEventStoreMock;
        private readonly IDomainEventHandler<UserRoomForRentAnnouncementPreferenceChangedDomainEvent> _domainEventHandler;

        public UserRoomForRentAnnouncementPreferenceChangedDomainEventHandlerTest()
        {
            _domainEventStoreMock = new Mock<IDomainEventStore>();
            _domainEventHandler = new UserRoomForRentAnnouncementPreferenceChangedDomainEventHandler(_domainEventStoreMock.Object);
        }
        ...
        public async Task HandleAsync_Should_Store_DomainEvent()
        {
            var roomForRentAnnouncementPreference = RoomForRentAnnouncementPreference.Builder()
                ...
                .Build();
            var domainEvent = new UserRoomForRentAnnouncementPreferenceChangedDomainEvent(Guid.NewGuid(),
            ...
            _domainEventStoreMock.Verify(x =>
                x.Store(It.Is<IDomainEvent>(e =>
                    e.GetType() == typeof(UserRoomForRentAnnouncementPreferenceChangedDomainEvent))));
        }
    }
}