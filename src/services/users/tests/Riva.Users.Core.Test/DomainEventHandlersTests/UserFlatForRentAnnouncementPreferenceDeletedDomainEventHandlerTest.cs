...
{
    public class UserFlatForRentAnnouncementPreferenceDeletedDomainEventHandlerTest
    {
        private readonly Mock<IDomainEventStore> _domainEventStoreMock;
        private readonly IDomainEventHandler<UserFlatForRentAnnouncementPreferenceDeletedDomainEvent> _domainEventHandler;

        public UserFlatForRentAnnouncementPreferenceDeletedDomainEventHandlerTest()
        {
            _domainEventStoreMock = new Mock<IDomainEventStore>();
            _domainEventHandler = new UserFlatForRentAnnouncementPreferenceDeletedDomainEventHandler(_domainEventStoreMock.Object);
        }
        ...
        public async Task HandleAsync_Should_Store_DomainEvent()
        {
            var flatForRentAnnouncementPreference = FlatForRentAnnouncementPreference.Builder()
                ...
                .SetRoomNumbersMax(1)
                .Build();
            var domainEvent = new UserFlatForRentAnnouncementPreferenceDeletedDomainEvent(Guid.NewGuid(),
            ...
            _domainEventStoreMock.Verify(x =>
                x.Store(It.Is<IDomainEvent>(e =>
                    e.GetType() == typeof(UserFlatForRentAnnouncementPreferenceDeletedDomainEvent))));
        }
    }
}