...
{
    public class UpdateFlatForRentAnnouncementPreferenceFunction
    {
        private readonly IFlatForRentAnnouncementPreferenceService _flatForRentAnnouncementPreferenceService;
        private readonly ILogger _logger;

        public UpdateFlatForRentAnnouncementPreferenceFunction(IFlatForRentAnnouncementPreferenceService flatForRentAnnouncementPreferenceService, ILogger logger)
        {
            _flatForRentAnnouncementPreferenceService = flatForRentAnnouncementPreferenceService;
            _logger = logger;
        }

        [FunctionName(ConstantVariables.UpdateFlatForRentAnnouncementPreferenceFunctionName)]
        public async Task Run(
            [ServiceBusTrigger(
                ConstantVariables.ServiceBusTopicName, 
                ConstantVariables.UpdateFlatForRentAnnouncementPreferenceSubscriptionName, 
        ...
        {
            var integrationEvent = JsonConvert.DeserializeObject<UserFlatForRentAnnouncementPreferenceUpdatedIntegrationEvent>(Encoding.UTF8.GetString(inputMessage.Body));

            try
            {
                await _flatForRentAnnouncementPreferenceService.UpdateAsync(integrationEvent);
